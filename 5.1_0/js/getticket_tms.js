    ...
    let icecubes_sys=()=>{
     ...
     chrome.storage.local.get([
        ...
        if(result.tms_using==0){
            ...
            let doselecttime=()=>{
                let tryselecttime=setInterval(()=>{
                    let datesplit=result.tms_date.split('-');
                    let dateselect=datesplit[0]+'.'+parseInt(datesplit[1])+"."+datesplit[2];
                    if(document.getElementById("DataPickup."+dateselect+"."+(parseInt(result.tms_start)<10?("0"+parseInt(result.tms_start)):parseInt(result.tms_start))+".1")!=null){
                        
                        for(let i=parseInt(result.tms_start);i<(parseInt(result.tms_end)+1);i++){
                            setTimeout(()=>{
                        
                            if(document.getElementById("DataPickup."+dateselect+"."+(i<10?("0"+i):i)+".1").getElementsByTagName("div")[0].innerText==""+(i<10?("0"+i):i)+":00 ~ "+((i+1)<10?("0"+(i+1)):(i+1))+":00" &&
                             !document.getElementById("DataPickup."+dateselect+"."+(i<10?("0"+i):i)+".1").getElementsByTagName("div")[0].attributes.class.value.includes("Booking"))
                             {
                                element=document.getElementById("DataPickup."+dateselect+"."+(i<10?("0"+i):i)+".1").getElementsByTagName("div")[0];
                        ...
                        clearInterval(tryselecttime);
                    }
                },1000);
            }
           
            let dotimeandplace=()=>{
                let trytimeandplace=setInterval(() => {

                    if(document.getElementById("PickupDateInterFaceBox").children.length!=0 && !document.getElementById("PickupDateInterFaceBox").children[0].innerHTML.includes("載入中 Loading")){
